...
public class KickedOutAuthorizationFilter extends AccessControlFilter {
    ...
    /***
     * 测试方式采用不同的browser进行测试,后面登录的同一账号会将第一个登录的账户踢下线
     * @param request
     * @param response
     * @return
     * @throws Exception
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        //2.1当前用户的队列
        RDeque<String> deque = redissonClient.getDeque(CacheConstant.GROUP_CAS + "kickedOutAuthorizationFilter:" + loginName);
        ...
        //（可以实现不同的业务逻辑，此处采用移除最先登录账号的session处理）

...
@MapperScan(basePackages = {"com.itheima.shiro.mapper", "com.itheima.shiro.mappercustom"},
        sqlSessionFactoryRef = "sqlSessionFactoryBeanDubboApp")
@EnableConfigurationProperties({DubboAppDataSourceProperties.class})
public class MybatisConfigDubboApp {


    @Autowired
    DubboAppDataSourceProperties dataSourceProperties;
    ...
    @Bean(name = "dataSourceDubboApp", initMethod = "init", destroyMethod = "close")
    public AtomikosDataSourceBean dataSourceShiro() {
        log.info("dataSourceDubboApp:抽象类创建开始!IP=" + IpAddrUtil.hostIpAddr());
    ...
    @Bean(name = "sqlSessionFactoryBeanDubboApp")

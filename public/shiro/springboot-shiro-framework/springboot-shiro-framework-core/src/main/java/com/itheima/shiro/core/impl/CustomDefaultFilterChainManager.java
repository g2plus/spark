...
public class CustomDefaultFilterChainManager extends DefaultFilterChainManager {
    ...
    @PostConstruct
    public void init(){
        //super.getFilters() 返回父类的Map地址 filter的个数为16的原因见shiroconfig类
        Map<String, Filter> defaultFilters = super.getFilters();
        //apply global settings if necessary:
        for (Filter filter : defaultFilters.values()) {
            //提取shirofactorybean中加在登录url信息的工作转为自定义CustomDefaultFilterChainManager的工作
    ...
    public void setCustomFilters(Map<String,Filter> customFilters){
        for (Map.Entry<String, Filter> entry : customFilters.entrySet()) {
            super.addFilter(entry.getKey(), entry.getValue(), false);
        }

    }

    /**
     * @Description 不再使用shiro容器去构建，而是交于spring去初始化filter
     */
    @Override
